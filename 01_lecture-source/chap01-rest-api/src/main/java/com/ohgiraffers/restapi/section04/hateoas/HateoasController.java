...
public class HateoasController {
    ...
    *      충분한 정보(링크) 를 제공함으로서 클라이언트가
    *      리소스(자원)에 대한 이해를 더욱 잘할 수 있도록 제공하는 것이다.
    *      RestFul API 라고도 한다. -> 정보가 충분한 API */
    ...
    public ResponseEntity<ResponseMessage> findAllUsers() {
        ...
        /* comment.
         *   EntityModel 란?
         *    - Hateoas 에서 제공하는 클래스로 RestAPI 를 구성할 때 해당 리소스와 연관이있는 링크를 포함할 수 있도록하는 것이다. */
        List<EntityModel<UserDTO>> userLink = users.stream().map(
                user -> EntityModel.of(
                        user,
                        // withSelfRel()
                        //  - 특정 리소스를 조회하는 URL 링크를 제공하며 해당 링크를 제공함으로서 리소스에 대한 정보를 알 수 있다.
                        linkTo(methodOn(HateoasController.class).findUserByNo(user.getNo())).withSelfRel(),
                        // withRel()
                        //  - 현재 리소스와 관련 된 관계를 나타낸다. 즉, user 한 명은 users 라는 리소스에 포함
